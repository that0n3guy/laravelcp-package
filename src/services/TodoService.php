...
use Gcphost\Laravelcp\Repositories\TodoRepository as Todos;
use Input, Lang, Redirect, Datatables;
...
class TodosService {
	...
	public function create(){
		...
		return count($errors->all()) == 0 ?
			(Api::to(array('success', Lang::get('Laravelcp::admin/todos/messages.create.success'))) ? : Redirect::to('admin/todos/' . $this->todo->id . '/edit')->with('success', Lang::get('Laravelcp::admin/todos/messages.create.success'))) : 
			(Api::to(array('error', Lang::get('Laravelcp::admin/todos/messages.create.error'))) ? : Redirect::to('admin/todos/create')->withErrors($errors));
	}
	...
	public function edit($todo){
		...
		return count($errors->all()) == 0 ?
			(Api::to(array('success', Lang::get('Laravelcp::admin/todos/messages.create.success'))) ? : Redirect::to('admin/todos/' . $todo->id . '/edit')->with('success', Lang::get('Laravelcp::admin/todos/messages.create.success'))) : 
			(Api::to(array('error', Lang::get('Laravelcp::admin/todos/messages.create.error'))) ? : Redirect::to('admin/todos/' . $todo->id . '/edit')->withErrors($errors));
	}
	...
	public function assign($todo){
        return $todo->assign() ? Api::json(array('result'=>'success')) : Api::json(array('result'=>'error', 'error' =>Lang::get('Laravelcp::core.delete_error')));
	}
